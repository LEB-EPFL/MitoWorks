...
function [Contour]=mitoProfiles(fname,CropData,start_frame,nframe,px,S,Norm,mesh,meshSpacing);

fname = '119ProbMap1-1.tif'; % name of mitochondria probability map
...
nframe = 22;% number of frames in movie before break
start_frame=19; %frame to start_frame analysis
...
for j=start_frame:nframe
   bw{:,j}=activecontour(I_filter(:,:,j),mask, 1500, 'Chan-Vese', 'SmoothFactor', 1.5);
   ...
   %smooth again
   Contour_final{:,j}=lssmooth(Contour_f{:,j}, S)
end
...
%  delB=IDX_normB-length(Norm{1,start_frame});
 ...
 if del<=0
   SelectSlice=Norm{1,start_frame}(IDX_norm,:); %coordinates of slice (first frame)
 ...
 ConIntDen= cellfun(@sum, IntDen_SelectCon);
 id1=IDX_norm;
 else
...
% BleachFit=polyfit((start_frame:1:nframe)', MitoIntDen_Select(start_frame:end)', 1);
% BleachFit=polyfit((start_frame:1:nframe)',  TotMitoIntDen(start_frame:end)', 1)
 BleachFit=polyfit((start_frame:1:nframe)',  ConIntDen(start_frame:end)', 1);
...
Sol{1,f}(rr,2)= (-B{1,f}(rr,:) + sqrt(B{1,f}(rr,:)*B{1,f}(rr,:) - 4*A{1,f}(rr,:)*C{1,f}(rr, :)))/(2*A{1,f}(rr,:)); 
